...
API_URL = "https://script.google.com/macros/s/AKfycbysQw-KRh2kDCHB0SsuOJ8EkBlDPKQtYkinj3mB9lKdYpIakHJZxPtQ9lpiq_20UAGi/exec"
EXCHANGE_RATE = 1470 
...
if not user_df.empty:
    ...
    for idx, row in user_df.iterrows():
        try:
            ...
            stocks_to_show.append({'row': row, 'curr': curr, 'hist': h, 'idx': idx})
        except: continue
    ...
    for item in stocks_to_show:
        r, curr, hist, i = item['row'], item['curr'], item['hist'], item['idx']
        buy_p, qty = float(r['buy_price']), float(r['qty'])
        diff = curr - buy_p
        rate = (diff / buy_p * 100) if buy_p > 0 else 0
        p_usd = diff * qty
        
        psi_val, supply_val = 72.0, 94.9 # 가상 지표
        s_color = "#00FF7F" if supply_val > 80 else "#FFA500"
        p_color = "#FF4B4B" if psi_val > 70 else ("#0066FF" if psi_val < 30 else "#FFFFFF")
