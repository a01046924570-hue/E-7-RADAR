...
st_autorefresh(interval=30000, limit=None, key="refresh_v25")
    ...
    with st.expander("➕ 타겟 신규 등록", expanded=True):
        with st.form("add_form", clear_on_submit=True):
            ...
            if st.form_submit_button("레이더 등록"):
                if t:
                    requests.post(API_URL, json={"action":"add", "user_id":user_pw, "type":target_type, "ticker":t, "buy_price":p, "qty":q})
                    st.rerun()
...
if not filtered_df.empty:
    ...
    for idx, row in filtered_df.iterrows():
        try:
            ...
            prev_close = h['Close'].iloc[-2] # 전일 종가
            day_change = ((curr - prev_close) / prev_close * 100) # 일일 등락률 계산
    ...
    for item in stocks_to_show:
        ...
        with c_info:
            ...
            st.markdown(f"""
                <div style='display:flex; align-items:baseline; gap:12px; margin-bottom:10px;'>
                    ...
                    <span style='color:{chg_color}; font-size:1.4rem; font-weight:bold;'>{day_change:+.2f}%</span>
                </div>
            """, unsafe_allow_html=True)
            ...
            if r['type'] == "보유주식":
                ...
                m3.markdown(f"<p class='metric-label'>내 수익률</p><p class='metric-val' style='color:{profit_color};'>{rate:+.2f}%</p><p class='sub-val'>₩{(curr-buy_p)*qty*EXCHANGE_RATE:,.0f}</p>", unsafe_allow_html=True)
            else:
        ...
        with c_chart:
            # 에러 해결: 고유 키(i)를 사용하여 차트 중복 방지
            fig = go.Figure(data=[go.Scatter(x=hist.index, y=hist['Close'], line=dict(color='#0066FF', width=3))])
            fig.update_layout(margin=dict(l=0,r=0,t=0,b=0), height=100, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(visible=False), yaxis=dict(visible=False))
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"chart_v2_{i}")
            ...
            with st.popover("⚙️ 관리", use_container_width=True):
                ...
                if st.button("업데이트", key=f"upd_{i}", use_container_width=True):
